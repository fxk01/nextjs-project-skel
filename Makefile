.PHONY: default up stop restart down build lint
...
up:
	@echo "${YELLOW}run containers...${COLOR_END}"
	docker run -it -p 127.0.0.1:3000:3000 nextjs:v0.1 npm run dev
...
knex1:
	docker-compose run postgres -D /usr/local/pgsql/data
